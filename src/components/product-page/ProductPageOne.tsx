...
const ProductPage = () => {
  ...
  const homeDecorProducts = useMemo(() => data?.filter((product) => product.category === 'Home Decor'), [data]);
  ...
  useEffect(() => {
    if (homeDecorProducts) {
      const newTypes = new Set(homeDecorProducts.map((product) => product.type));
  ...
  const isLoadMoreDisabled = homeDecorProducts ? displayedProducts >= homeDecorProducts.length : true;
  ...
  const filteredProducts = selectedType !== null ? homeDecorProducts?.filter((product) => product.type === selectedType) : homeDecorProducts;
